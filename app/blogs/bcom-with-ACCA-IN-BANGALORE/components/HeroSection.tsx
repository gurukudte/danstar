"use client"
import { Briefcase, Globe, BarChart, Award } from "lucide-react";
import { Button } from "@/components/ui/button";
import { motion } from "framer-motion";

export default function HeroSection() {
  const highlights = [
    { icon: <Briefcase className="w-6 h-6" />, text: "Global Certifications" },
    { icon: <Globe className="w-6 h-6" />, text: "Big 4 Opportunities" },
    { icon: <BarChart className="w-6 h-6" />, text: "₹6-12 LPA Salary" },
    { icon: <Award className="w-6 h-6" />, text: "Industry Partnerships" },
  ];
    ...
    <section className="relative overflow-hidden bg-gradient-to-br from-primary to-secondary py-16 md:py-24 lg:py-32">
      <div className="flex flex-col items-center relative z-10 px-4 md:px-6">
        <motion.div
          ...
          className="mx-auto max-w-6xl text-center space-y-6"
        >
          ...
          <p className="text-xl text-white md:text-2xl">
            Upgrade your commerce degree with global certifications and
            corporate training
          </p>
